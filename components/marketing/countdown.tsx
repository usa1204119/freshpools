...
export function Countdown({
  ...
  if (remaining.done) {
    return (
      <p className={cn("mono border border-ink bg-paper px-4 py-3 text-label", className)}>
        {closedLabel}
      </p>
    );
  }
  ...
  return (
    <div className={className}>
      ...
      <ul className="flex w-max border border-ink">
        {units.map((unit) => (
          <li
            key={unit.label}
            className="min-w-16 border-r border-ink px-3 py-2 text-center last:border-r-0"
          >
